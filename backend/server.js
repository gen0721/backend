...
if (fs.existsSync(frontendDist)) {
  ...
  console.log('⚠️ Frontend dist directory not found, serving API only');
}
...
initSchema()
  ...
  .then(async () => {
    // Dev-only test user
    const { ensureTestUser } = require('./utils/seedTestUser');
    const r = await ensureTestUser().catch(e => ({ ok: false, error: e.message }));
    if (r?.ok === false) console.warn('⚠️  Test user seed failed:', r.error);
    else if (r?.created) console.log(`✅ Test user created: @${r.username} (set TEST_USER_PASSWORD to change)`);
    else if (r?.updated) console.log(`✅ Test user updated: @${r.username}`);
  })
  .then(() => {
    app.listen(PORT, () => {
