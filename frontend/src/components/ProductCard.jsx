...
export default function ProductCard({ product, style={} }) {
  ...
  const showSheen = hovered || !!product.isPromoted
    ...
    >
      ...
      >
        <div
          ...
          style={{
            cursor:'pointer',
            background:'radial-gradient(circle at 0 0, rgba(245,200,66,0.06), transparent 55%), var(--bg2)',
        ...
        >
          <div
            className={`img-sheen ${showSheen ? 'img-sheen--active' : ''}`}
            style={{
              height:160,
              background:'var(--bg3)',
              position:'relative',
          ...
          >
            ...
            {product.isPromoted && (
              <div style={{ position:'absolute', top:10, left:10 }}>
                <span className="badge badge-yellow" style={{ boxShadow:'0 12px 28px rgba(245,200,66,0.15)', backdropFilter:'blur(10px)' }}>🚀 ТОП</span>
              </div>
            )}
            <div style={{ position:'absolute', inset:0, background:'radial-gradient(circle at 20% 0%, rgba(255,255,255,0.12), transparent 55%)', opacity: hovered ? 0.8 : 0.55, transition:'opacity 0.3s ease' }}/>
            <div style={{ position:'absolute', bottom:0, left:0, right:0, height:70, background:'linear-gradient(transparent, rgba(5,5,16,0.92))' }}/>
          ...
          <div style={{ padding:'14px 16px' }}>
            <div className="clamp-1" style={{ fontFamily:'var(--font-h)', fontWeight:800, fontSize:15, marginBottom:6, letterSpacing:'-0.01em' }}>
              {product.title}
            </div>
            <div className="clamp-1" style={{ fontSize:12, color:'var(--t3)', marginBottom:12 }}>
