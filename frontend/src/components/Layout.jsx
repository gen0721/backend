...
const IconHome     = () => <Home size={22} strokeWidth={1.75} className="icon-animated icon-animated--subtle"/>
const IconSearch   = () => <Search size={22} strokeWidth={1.75} className="icon-animated icon-animated--subtle"/>
const IconGrid     = () => <LayoutGrid size={22} strokeWidth={1.75} className="icon-animated icon-animated--subtle"/>
const IconPlus     = () => <Plus size={26} strokeWidth={2.5} className="icon-animated"/>
const IconMessages = () => <MessageCircle size={22} strokeWidth={1.75} className="icon-animated icon-animated--subtle"/>
const IconProfile  = () => <UserCircle size={22} strokeWidth={1.75} className="icon-animated icon-animated--subtle"/>
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      ...
      <header style={{
        ...
        background: scrolled ? 'rgba(10,10,18,0.86)' : 'rgba(10,10,18,0.55)',
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', height:64, display:'flex', alignItems:'center', gap:12 }}>
          ...
          <Link to="/" style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            <div style={{
              ...
              background:'radial-gradient(circle at 30% 0%, #fffbe6, #f5c842 40%, #e8500a 100%)',
              display:'flex', alignItems:'center', justifyContent:'center',
              fontSize:20,
              boxShadow:'0 0 25px rgba(245,200,66,0.7)',
              animation:'float-soft 3.4s ease-in-out infinite, pulse-glow 4.8s ease-out infinite'
            }}>M</div>
          ...
          {/* Desktop nav */}
          <nav style={{ display:'flex', alignItems:'center', gap:6, flex:1, marginLeft:10 }} className="desktop-nav">
            {[{ to:'/', label:'Главная' }, { to:'/catalog', label:'Каталог' }].map(n => (
              <Link key={n.to} to={n.to} style={{
                ...
                background: isActive(n.to)
                  ? 'linear-gradient(135deg, rgba(245,200,66,0.16), rgba(124,106,255,0.10))'
                  : 'rgba(255,255,255,0.03)',
                border: `1px solid ${isActive(n.to) ? 'rgba(245,200,66,0.25)' : 'rgba(255,255,255,0.06)'}`,
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }} className="desktop-actions">
            {user ? (
              <>
                <Link to="/sell" className="btn btn-sm btn-secondary" style={{ borderRadius:999 }}>+ Продать</Link>
                <div style={{ position:'relative' }}>
                  <button onClick={() => setMenuOpen(!menuOpen)} style={{
                    display:'flex', alignItems:'center', gap:8, padding:'6px 12px',
                    background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.10)', borderRadius:14,
                  ...
                  {menuOpen && (
                    <div onClick={() => setMenuOpen(false)} style={{ position:'fixed', inset:0, zIndex:50 }}>
                      <div onClick={e => e.stopPropagation()} style={{
                        ...
                        background:'rgba(16,16,27,0.80)', border:'1px solid rgba(255,255,255,0.10)',
                        borderRadius:20, padding:8, minWidth:190,
                        boxShadow:'0 24px 90px rgba(0,0,0,0.65)', zIndex:51,
                        animation:'fadeUp 0.2s ease'
                      }}>
                        {[
                          { to:'/profile', icon:'', label:'Профиль' },
                          { to:'/wallet',  icon: <Wallet size={16} strokeWidth={1.75} className="icon-animated--subtle"/>, label:'Кошелёк' },
                          { to:'/deals',   icon: <Handshake size={16} strokeWidth={1.75} className="icon-animated--subtle"/>, label:'Сделки' },
                        ].map(item => (
                        ...
                        {(user.isAdmin || user.isSubAdmin) && (
                          <Link to="/admin" style={{
                            ...
                            <Zap size={13} strokeWidth={2} className="icon-animated"/> Админка
                          </Link>
                        )}
              ...
              <>
                ...
                <Link to="/auth?mode=register" className="btn btn-sm btn-primary" style={{ borderRadius:999 }}>Регистрация</Link>
              </>
            )}
          </div>
          ...
          <div style={{ display:'none', alignItems:'center', gap:10, marginLeft:'auto' }} className="mobile-header-right">
            {user && (
              <div style={{
                ...
                <DollarSign size={13} strokeWidth={2} className="icon-animated icon-animated--subtle" style={{marginRight:2}}/>${parseFloat(user.balance||0).toFixed(2)}
              </div>
            )}
            <button
              onClick={() => setMobileMenu(true)}
              style={{
                width:40, height:40, borderRadius:14, background:'rgba(255,255,255,0.04)',
      ...
      {mobileMenu && (
        <>
          ...
          <div style={{
            ...
            background:'rgba(16,16,27,0.86)', borderLeft:'1px solid rgba(255,255,255,0.10)',
