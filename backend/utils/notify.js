...
async function notifyDealComplete(buyer, seller, productTitle, sellerAmount) {
  await Promise.all([
    sendTg(buyer.telegram_id || buyer.telegramId, `✅ <b>Сделка завершена</b>\n\nТовар: ${productTitle}\nСделка успешно закрыта.`),
    sendTg(seller.telegram_id || seller.telegramId, `✅ <b>Деньги зачислены</b>\n\nТовар: ${productTitle}\nЗачислено: <b>+$${parseFloat(sellerAmount).toFixed(2)}</b> (после комиссии 5%)`),
  ]);
}

async function notifyDealDispute(buyer, seller, productTitle) {
  await Promise.all([
    sendTg(buyer.telegram_id  || buyer.telegramId,  `⚠️ <b>Спор открыт</b>\n\nТовар: ${productTitle}\nАдмин рассмотрит спор.`),
    sendTg(seller.telegram_id || seller.telegramId, `⚠️ <b>Покупатель открыл спор</b>\n\nТовар: ${productTitle}\nАдмин рассмотрит спор.`),
  ]);
}
...
async function notifyUnbanned(user) {
  await sendTg(user.telegram_id || user.telegramId, `✅ <b>Аккаунт разблокирован</b>\n\nДобро пожаловать обратно!`);
}
